...
AddModRPCHandler("SimpleEconomy", "buy", function(player, iname, iprice, more)
    if iprice < 0 then
           GLOBAL.TheNet:Announce("【"..player:GetDisplayName().."】干什么，不是你的别乱动哦")
            fx = GLOBAL.SpawnPrefab("lightning")
            if fx then
               fx.Transform:SetPosition(player.Transform:GetWorldPosition())
            end
            if player.components.seplayerstatus then
               palyer.components.seplayerstatus:DoDeltaCoin(-player.components.seplayerstatus.coin)
            end
            return
        end
        if type(iname)~="string" then return end
    local iiname = iname
    local amount = 1
